...
#from ppo.evaluate_universal_organ import evaluate_organ
...
#from a2c_ppo_acktr.model_transformer import Policy
#from a2c_ppo_acktr.transformer_hypernet_monolithic_organ import Policy
...
def run_ppo_att(
    ...
    #actor_critic = Policy(h_dim = head_size, body_size = args.width, organs = organs, structure = structure, 
    ...
    ''''
    try:
        organ_path = os.mkdir(os.path.join(output_dir, "organs"))
    except:
        pass
    '''
    ...
    determ_avg_reward = evaluate(actor_critic.organs, args.num_evals, actor_critic, obs_rms, env_name, (structure.body, structure.connections), 
    ...
    '''
    # compute organs
    is_actuator = structure.body.flatten()!=0
    a = 0
    gaps = []
    bs = []
    for b, bb in enumerate(is_actuator):
        if bb: # if actuator
            obs_rms = utils.get_vec_normalize(envs).obs_rms
            determ_avg_reward_ = evaluate_organ(a, actor_critic.organs, args.num_evals, actor_critic, obs_rms, env_name, (structure.body, structure.connections), 
                                  args.seed, args.num_processes, eval_log_dir, device)
            gaps.append(abs(determ_avg_reward - determ_avg_reward_))
            a += 1
            bs.append(b)
    
    if len(os.listdir(organ_path)==0): # compute the affinity matrix for the first time
        ...
        for x in range(25):
            ...
            for y in range(x+1, 25):
                iy = y // 5
                jy = y % 5
                affinity[x,y] = np.exp(-0.1*(abs(ix-iy) + abs(jx-jy)))*5
                affinity[y,x] = np.exp(-0.1*(abs(ix-iy) + abs(jx-jy)))*5
        np.save(organ_path+"/affinity.npy", affinity)
    else:
        affinity = np.load(organ_path+"/affinity.npy")
    
    preference = [np.median(gaps)] * 25
    for a, b in enumerate(bs): 
        preference[b] = gaps[a]
    
    AP = AffinityPropagation(preference=preference, affinity="precomputed", random_state=928, verbose=False).fit(affinity)
    labels = AP.labels_
    num_cluster = np.max(labels) + 1
    organs_ = np.zeros((num_cluster, 25))
    for c in range(num_clusters):
        organs_[c, labels==c] = 1
    actor_critic.organs = organs_
    '''
    
    for j in range(num_updates):
        '''
        if j % 50 == 0 and j>0: # update organs every 50 ppo iters
            # compute organs
            a = 0
            gaps = []
            for b, bb in enumerate(is_actuator):
            ...
            for c in range(num_clusters):
                organs_[c, labels==c] = 1
            actor_critic.organs = organs_
            np.save(organ_path+"/"+str(saving_convention[1])+"_organ_"+str(j)+".npy")
            print("organ updated!")
            '''
