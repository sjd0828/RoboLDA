"""MetaMorph + RoboLDA organ masks (our method).

Loads morphologies and organ assignments from pre-generated .pt files,
converts organs to one-hot representation, creates a shared Transformer
policy (metamorph_organ.ATTBase), and trains via parallel PPO.
"""
...
def parse_control_args():
    parser = argparse.ArgumentParser(
        description="MetaMorph + RoboLDA organ masks control experiment")
    ...
    parser.add_argument("--organ-dir", type=str, required=True,
                        help="Directory containing organ.pt")
...
def run_universal(structures, organs, ppo_args, tc, save_dir, output_dir, temp_path):
    ...
    group = mp.Group()
    for idx, (structure, organ) in enumerate(zip(structures, organs)):
        job_args = (
...
if __name__ == "__main__":
    ...
    if ctrl_args.output_name is None:
        ...
        exp_name = "{}-metamorph-organ-{}".format(task_short, timestamp)
    else:
    ...
    origin_organs = torch.load(os.path.join(ctrl_args.organ_dir, "organ.pt"))[indices, :, :]
    organ_num = int(origin_organs.max().item()) + 1
    organs = torch.zeros(origin_organs.shape[0], organ_num, 25)
    for r in range(origin_organs.shape[0]):
        for i in range(5):
            for j in range(5):
                organs[r, origin_organs[r, i, j].long(), i * 5 + j] = 1
    torch.save(origin_organs, os.path.join(save_dir, "lda_organ.pt"))
    print("Organs loaded: shape {}, {} organ types.".format(
        list(organs.shape), organ_num))

    run_universal(structures, organs, ppo_args, tc, save_dir, output_dir, temp_path)
