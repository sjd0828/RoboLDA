...
def make_env(env_id, robot_structure, seed, rank, log_dir, allow_early_resets):
    def _thunk():
        if env_id.startswith("dm"):
            _, domain, task = env_id.split('.')
            env = dm_control2gym.make(domain_name=domain, task_name=task)
        else:
            env = gym.make(env_id, body=robot_structure[0], connections=robot_structure[1])
...
def make_vec_envs(env_name,
    ...
    envs = [
        make_env(env_name, robot_structure, seed, i, log_dir, allow_early_resets)
