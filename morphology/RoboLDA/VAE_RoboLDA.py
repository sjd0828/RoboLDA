...
class RoboLDA(nn.Module):
    """Hierarchical probabilistic generative model: task -> individual -> organ -> voxel."""

    def __init__(self, task_num, individual_num, organ_num, comp_num, voxel_num,
                 robot_num, hidden, dropout):
        super().__init__()
        ...
        self.task_bn = nn.BatchNorm1d(task_num, affine=False)
        ...
        self.ind_bn = nn.BatchNorm1d(individual_num, affine=False)

        # voxel+individual -> organ
        ...
        self.organ_bn = nn.BatchNorm1d(organ_num * 25, affine=False)
    ...
    def model(self, robos, tasks, logtheta1=None, logtheta2=None,
              logtheta3=None, logtheta4=None):
        pyro.module("ss_vae", self)

        log_theta_1 = pyro.sample(
            "logtheta1",
            dist.Normal(torch.zeros(self.task_num),
                        torch.ones(self.task_num)).to_event(1),
            obs=logtheta1)
        log_theta_1 = self.drop(F.softmax(log_theta_1.reshape(1, self.task_num), -1))

        log_theta_2 = pyro.sample(
            "logtheta2",
            dist.Normal(torch.zeros(self.task_num * self.individual_num),
                        torch.ones(self.task_num * self.individual_num)).to_event(1),
            obs=logtheta2)
        log_theta_2 = self.drop(F.softmax(
            log_theta_2.reshape(self.task_num, self.individual_num), -1))

        log_theta_3 = pyro.sample(
            "logtheta3",
            dist.Normal(
                torch.zeros(self.individual_num * self.organ_num * self.comp_num),
                torch.ones(self.individual_num * self.organ_num * self.comp_num)
            ).to_event(1),
            obs=logtheta3)
        log_theta_3 = self.drop(F.softmax(
            log_theta_3.reshape(self.comp_num, self.individual_num, self.organ_num), -1))

        log_theta_4 = pyro.sample(
            "logtheta4",
            dist.Normal(
                torch.zeros(self.organ_num * self.voxel_num * 25),
                torch.ones(self.organ_num * self.voxel_num * 25)
            ).to_event(1),
            obs=logtheta4)
        log_theta_4 = self.drop(F.softmax(
            log_theta_4.reshape(25, self.organ_num, self.voxel_num), -1))
        ...
        with pyro.plate("robots"):
            ...
            tasks = pyro.sample(
                "tasks",
                dist.OneHotCategorical(probs=task_params).to_event(1),
                obs=tasks)

            individual_params = torch.mm(tasks, log_theta_2)
            individuals = pyro.sample(
                "individuals",
                dist.OneHotCategorical(probs=individual_params).to_event(1))

            temp = torch.cat([
                individuals.unsqueeze(1).repeat(1, 25, 1),
                self.voxel_location.unsqueeze(0).repeat(n, 1, 1)
            ], dim=2)
            organ_weight = self.activation(self.organ_weight_1(temp))
            organ_weight = self.activation(self.organ_weight_2(organ_weight))
            organ_weight = self.organ_weight_3(organ_weight)
            temp = log_theta_3.reshape(self.comp_num, -1).unsqueeze(0).repeat(n, 1, 1)
            temp = torch.bmm(organ_weight, temp).reshape(
                n, 25, self.individual_num, self.organ_num)
            temp = torch.bmm(
                individuals.unsqueeze(1).repeat(1, 25, 1).unsqueeze(2).reshape(
                    n * 25, 1, self.individual_num),
                temp.reshape(n * 25, self.individual_num, self.organ_num))
            temp = temp.squeeze(1).reshape(n, 25, self.organ_num)
            organs = pyro.sample(
                "organs",
                dist.OneHotCategorical(logits=temp).to_event(2))

            voxel_params = torch.bmm(
                organs.permute(1, 0, 2), log_theta_4).permute(1, 0, 2)
            if robos is not None:
                voxels = pyro.sample(
                    "voxels",
                    dist.OneHotCategorical(probs=voxel_params).to_event(2),
                    obs=robos.reshape(-1, 25, 5))
            else:
                voxels = pyro.sample(
                    "voxels",
                    dist.OneHotCategorical(probs=voxel_params).to_event(2),
    ...
    def guide(self, robos, tasks):
        ...
        with pyro.plate("data"):
            logtheta1_h = self.activation(
                self.logtheta1_fc1(torch.cat((robos, tasks), axis=1).reshape(1, -1)))
            ...
            logtheta1 = pyro.sample(
                "logtheta1",
                dist.Normal(logtheta1_loc, (0.5 * logtheta1_scale).exp()).to_event(1))

            logtheta2_h = self.activation(
                self.logtheta2_fc1(torch.cat((robos, tasks), axis=1).reshape(1, -1)))
            ...
            logtheta2 = pyro.sample(
                "logtheta2",
                dist.Normal(logtheta2_loc, (0.5 * logtheta2_scale).exp()).to_event(1))

            logtheta3_h = self.activation(
                self.logtheta3_fc1(torch.cat((robos, tasks), axis=1).reshape(1, -1)))
            ...
            logtheta3 = pyro.sample(
                "logtheta3",
                dist.Normal(logtheta3_loc, (0.5 * logtheta3_scale).exp()).to_event(1))

            logtheta4_h = self.activation(
                self.logtheta4_fc1(torch.cat((robos, tasks), axis=1).reshape(1, -1)))
            ...
            logtheta4 = pyro.sample(
                "logtheta4",
                dist.Normal(logtheta4_loc, (0.5 * logtheta4_scale).exp()).to_event(1))
        ...
        with pyro.plate("robos"):
            ...
            tasks = pyro.sample(
                "tasks",
                dist.OneHotCategorical(logits=task_logits).to_event(1),
                obs=tasks)

            ind_h = self.activation(self.ind_fc1(torch.cat(
                (robos, tasks, logtheta2.unsqueeze(0).repeat(robos.shape[0], 1)), dim=1)))
            ind_h = self.activation(self.ind_fc2(ind_h))
            ind_h = self.drop(ind_h)
            ind_logits = self.ind_bn(self.ind_fc3(ind_h))
            inds = pyro.sample(
                "individuals",
                dist.OneHotCategorical(logits=ind_logits).to_event(1))

            organ_h = self.activation(self.organ_fc1(torch.cat(
                (robos, inds, logtheta3.unsqueeze(0).repeat(robos.shape[0], 1)), dim=1)))
            ...
            organs = pyro.sample(
                ...
                dist.OneHotCategorical(
                    logits=organ_logits.reshape(-1, 25, self.organ_num)).to_event(2))

        return (logtheta1_loc, logtheta2_loc, logtheta3_loc, logtheta3_scale,
                logtheta4_loc, logtheta4_scale, inds, organs, organ_logits)

    def reset(self, robot_num):
        """Re-initialize logtheta encoder layers for a new dataset size."""
